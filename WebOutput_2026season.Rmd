...
Draft <- read.csv("DraftResults26.csv", stringsAsFactors = F)
Draft$Team <- factor(Draft$Team, levels = Draft$Team[1:10])
#
# Read NFL Standings
url <- "http://www.espn.com/nfl/standings/_/season/2026/group/league"
...
Schedule <- week_sched <- nflreadr::load_schedules(seasons = 2026) %>% filter(week == params$week)
...
expanded.standings <- left_join(nfl.standings[, 1:2], Draft[, 3:4], by = c("Team" = "Selection")) %>%
  ...
  mutate(Opponent = replace_na(Opponent, "--")) %>%
...
expanded.standings %>%
  ...
  kable(col.names = names_broken, escape = FALSE, format = "html") %>%
  kable_styling(bootstrap_options = c("hover"), full_width = F, position = "left") %>%
#  column_spec(1:ncol(expanded.standings), extra_css = "text-align:center;") %>%
...
sched_upcoming <- nflreadr::load_schedules(seasons = 2025) %>% 
...
pool.rosters %>%
  ...
  kable(col.names = names_broken, escape = F, format = "html") %>%
  kable_styling(bootstrap_options = c("hover"), full_width = F, position = "left") %>%
